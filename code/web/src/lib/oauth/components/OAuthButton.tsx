...
export const OAuthButton: React.FC<OAuthButtonProps> = ({ providerConfig, children }) => {
  const { initiateOAuth } = useOAuth(providerConfig);
    ...
    switch (providerConfig.name) {
      ...
      case "apple":
        return {
          ...
          logo: (
            <svg xmlns="http://www.w3.org/2000/svg" width="22px" fill="#fff" className="inline mr-3" viewBox="0 0 22.773 22.773">
              <path
  ...
  const { className, logo } = getBrandStyles();

  return (
    <button type="button" onClick={initiateOAuth} className={`w-96 flex items-center justify-center gap-2 ${className}`}>
      {logo}
      {children}
    </button>
  );
